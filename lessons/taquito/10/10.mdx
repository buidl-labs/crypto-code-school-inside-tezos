...
.then(storage => {
    console.log(storage.ledger) // will output a random string.
})
```
...
2. Access and return the value of your account in the `ledger`. Use `storage.ledger.get(account_address)` with your account's address as `account_address`