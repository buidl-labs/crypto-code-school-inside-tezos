...
---
### Intro
In this module, we'll take you through the whole process of originating(or deploying) a smart contract on the Tezos blockchian using Javascript.
...
### Study time
Taquito is a library that makes it really easy to make DApps around the Tezos blockchain.
...
The code we'll write can work both in the browser and the Node environment, so we won't show you how to wire this up with the UI or a CLI but you can essentially drop the code right into a React or Vue app and it should work no issues.
...
1. Install `taquito` using npm or yarn.
2. Import `Tezos` from `@taquito/taquito`
3. Generate your faucet account - 
