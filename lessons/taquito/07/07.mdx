...
editor:
  ...
  startingCode: |
    Tezos.contract.at("<CONTRACT_ADDRESS>")
        ...
        .then(hash => {
            // Log to the console - Operation injected: ${hash}
...
---
### Intro
The smart contract for `Cyber_Token` that we've originated has en entry point called `mint`, through which we can create new tokens. 
...
- Mint 10 tokens of the symbol `"CBT"` and token id `0` to your address(the pkh you obtained from the faucet data); To look at what parameter the entry point `mint` accepts and in what order, take a look at [this chapter](https://hackmd.io/@manangouhari/FA2-4).
    ...
    4. When you finally recieve the `hash` of the operation, log to the console: `Operation injected: {hash}`.