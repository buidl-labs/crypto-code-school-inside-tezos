...
Previously you had given the name of `terminator` to our `CryptoBot` contract. But then, if you come up with a cooler name in the future, how do you go about changing the CryptoBot’s name? 
...
2. Test your newly created `change_name` function by calling `change_name` on `crypto_bot_test_contract`. 
