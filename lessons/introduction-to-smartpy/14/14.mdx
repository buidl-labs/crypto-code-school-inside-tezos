...
You should already be pretty familiar with basic decision making in Python using `if...else` statements.  
...
In the later half of the chapter, we will design an alerting system to ping us when our `Cryptobot` runs out of plasma bullets! 
...
1. In smart contracts, most of decision making tends to happen via placing conditions over values stored inside the contract's storage. 
    * Since decision making evaluation in smart contracts actually happens when smart contracts are being simulated or finally deployed to the Tezos blockchain, `sp.if` let's us execute decision making with the contract's latest values in it's storage. 
     * Since plain `if` doesn't have SmartPy's superpowers, you won't be able to bet whether it is executing on the contract's latest updated values or not. 
...
Let's use `sp.if` to proceed forward with shooting only when `plasma_bullet_count` is greater than or equal to 1. If not, we will use `sp.failwith` to alert you that you run out of bullets!
