  ...
  startingCode: |
    ...
    class CryptoBot(sp.Contract):
        ...
        def shoot_alien(self, alien_type):
            ...
            # Using sp.if add a condition to check bullet_gem_count >= 1
...
In later half of the chapter, we design an alerting system to ping us when our `CryptoBot` runs out of Bullet gems! 
...
Let's use `sp.if` to proceed forward with shooting only when `bullet_gem_count` is greater than 1. If not, we will use `sp.failwith` to alert you that you run out of bullets!

1. Using `sp.if` add a condition to check `bullet_gem_count` >= 1. Use `self.data.bullet_gem_count` to access it. 
