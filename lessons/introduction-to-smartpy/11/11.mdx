    ...
    class CryptoBot(sp.Contract):
        def __init__(self, life_state):
            self.init(
                ...
                # Add bullet_gem_count with 5 bullets:
...
```python
## record_alien_kills is a variable that records your kill count for various types of aliens. 
self.init(
            record_alien_kills = {
                    "simple_alien": sp.nat(0), 
                    "boss_alien": sp.nat(0)
...
Your `CryptoBot` seems to be missing the most important ability: the ability to kill! Without it your CryptoBot is as good as dead 💀
...
1. Add `bullet_gem_count ` to our contract's storage and initialize with <span class="string-highlight">5</span> bullets. 
...
3. Complete the `shoot_alien` function's code by adding code: 
    * to reduce `bullet_gem_count` by <span class="string-highlight">1</span>.
...
Let us test our `shoot_alien` function by  shoot <span class="string-highlight">2</span> bullet gems, <span class="string-highlight">1</span> aimed at simple alien and <span class="string-highlight">1</span>  aimed towards boss alien respectively: 
