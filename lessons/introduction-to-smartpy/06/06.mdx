...
1. Add a test with the **name of “Test CryptoBot Contract”** using `@sp.add_test` decorator. 
...
4. Initialize an instance of the `CryptoBot` contract and store it inside `crypto_bot_test_contract`
5. Use the initialized instance of `CryptoBot` stored in `crypto_bot_test_contract`  and add it to the scenario created in step 3.
