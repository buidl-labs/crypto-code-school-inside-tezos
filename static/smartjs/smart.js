...
function ppInvocationResult(result) {
  ...
  try {
    var contents = result["results"]["contents"];
  ...
  return {message: returnMessage, contract: originatedContract}
}
...
class SmartmlJS{
  ...
  importValue(value){
    if (isinstance(value, str))
      return this.importSimpleValue("string", value);
    if (isinstance(value, int))
      return this.importSimpleValue("int", str(value));
    if (isinstance(value, Record))
      return this.ctx.call("importValue", value.export());
    if (isinstance(value, SpExpr))
      return this.ctx.call("importValue", value.export());
  ...
  parseMich(code){
    if (isinstance(code, str))
      return this.ctx.call("mich.string", code);
    if (hasattr(code, 'int'))
      return this.ctx.call("mich.int", code.int);
    if (hasattr(code, 'string'))
      return this.ctx.call("mich.string", code.string);
    ...
    if (hasattr(code, 'args'))
      args = code.args.map(arg=>this.parseMich(arg));
    else
      args = [];
